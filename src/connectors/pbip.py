...
class PowerBIPBIPConnector:
    ...
    def get_model_schema(self) -> Dict[str, Any]:
        """
        Parse TMDL files to extract tables, columns, and measures.

        Returns:
            Dict with 'tables', 'columns' (per table), and 'measures'
        """
        if not self.current_project:
            return {"error": "No project loaded", "tables": [], "columns": {}, "measures": []}

        tables = []
        columns = {}  # {table_name: [column_names]}
        measures = []  # [{name, table}]

        # Regex patterns for TMDL parsing
        table_pattern = re.compile(r"^table\s+'([^']+)'|^table\s+(\S+)", re.MULTILINE)
        column_pattern = re.compile(r"^\tcolumn\s+'([^']+)'|^\tcolumn\s+(\S+)", re.MULTILINE)
        measure_pattern = re.compile(r"^\tmeasure\s+'([^']+)'|^\tmeasure\s+(\S+)", re.MULTILINE)
        ...
        for tmdl_file in self.current_project.tmdl_files:
            try:
                ...
                if table_match:
                    ...
                    for measure_match in measure_pattern.finditer(content):
                        measure_name = measure_match.group(1) or measure_match.group(2)
                        measures.append({"name": measure_name, "table": table_name})

            except Exception as e:
                logger.warning(f"Could not parse {tmdl_file}: {e}")

        return {
            "tables": sorted(tables),
            "columns": columns,
            "measures": sorted(measures, key=lambda x: x["name"])
        }
